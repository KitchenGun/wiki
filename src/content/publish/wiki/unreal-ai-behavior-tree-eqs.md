...
AI는 플레이어가 의도를 읽을 수 있는 상태 전환과, 개발자가 원인을 추적할 수 있는 구조를 동시에 갖춰야 한다. 저는 그 기준으로 Behavior Tree, Blackboard, AI Perception, EQS를 조합해 추적·수색·연출 타이밍을 분리했다.
...
AI는 단순 추적보다 상태 전환과 연출 타이밍이 중요하다. UI나 옵션과 달리 플레이어가 의도를 바로 읽기 어렵기 때문에, 상태와 감지 근거를 확인할 수 있게 구성했다.
