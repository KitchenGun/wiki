...
Hermes Personal PT는 제가 반복 기록에서 누락과 추측을 줄이기 위해 만든 개인 자동화다. Discord 입력을 구조화된 운동·InBody 기록으로 바꾸고, 누적 데이터는 다음 루틴의 초안을 만드는 근거로만 사용한다.

목표는 트레이너처럼 모든 판단을 대신하는 것이 아니라, 기록 누락을 줄이고 최근 수행 이력·체성분 추세·회복 상태를 근거로 다음 루틴의 초안을 빠르게 만드는 것이다. 외부 일정 변경은 명시적 승인을 받았을 때만 실행한다.
