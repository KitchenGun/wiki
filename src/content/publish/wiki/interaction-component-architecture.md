...
반복되는 퍼즐 기믹은 기획 변경이 잦기 때문에, 저는 상속 계층을 늘리기보다 컴포넌트와 인터페이스로 역할을 분리했다. 신규 기믹을 만들 때 공통 상호작용 로직을 다시 작성하지 않고 조립할 수 있게 하는 것이 목표였다.
...
포트폴리오에서는 다이얼, 레버, 자물쇠, 키패드처럼 반복되는 퍼즐 기믹을 동일한 컴포넌트와 인터페이스 구조로 구현한 사례를 확인할 수 있습니다.
