...
Personal Hermes Agent는 제가 반복적인 개발·개인 업무를 자동화할 때, 입력 정규화와 실행 경계를 먼저 설계한 방식을 공개 가능한 형태로 정리한 저장소다. 게임 클라이언트에서 상태와 예외를 분리하듯, 에이전트도 수집·판단·실행·승인을 분리해 운영했다.
...
이 프로젝트는 [[unreal-mcp]]와 함께 AI 도구를 실제 개발 루틴에 연결하려는 개인 R&D 흐름에 속한다. 자동화 결과는 결론이 아니라, 사람이 더 빠르게 검토할 수 있는 입력으로 남긴다.
