export const projects = [
  {
    title: '골목길: 귀흔',
    displayTitle: '골목길: 귀흔 (PC/STOVE, 출시 2025.10.29, 2024.04.09~2026.05.01)',
    description: 'Unreal Engine PC horror project distributed by Smilegate. Worked on AI, Common UI, options, inventory, localization, audio, sequence, and puzzle/content systems.',
    role: 'Client Programmer',
    period: '2024.04.09~2026.05.01',
    platform: 'PC/STOVE',
    tags: ['Unreal Engine', 'AI', 'Common UI', 'UI', 'Localization', 'MetaSound', 'Sequencer', 'DLSS/FSR'],
    status: 'Released',
  },
  {
    title: 'Forbidden Art',
    displayTitle: 'Forbidden Art (PC VR/Steam/STOVE, Steam 출시 2024.03.25, 2023.04.15~2024.05.02)',
    description: 'Unreal Engine PC VR horror project. Implemented puzzle logic, input, inventory, Steam achievements, Depot/build operations, AI, localization, and options.',
    role: 'Client Programmer',
    period: '2023.04.15~2024.05.02',
    platform: 'PC VR/Steam/STOVE',
    tags: ['Unreal Engine', 'Steamworks', 'Puzzle', 'AI', 'VR'],
    status: 'Released',
  },
  {
    title: 'LAST MAG',
    description: 'Unity WebGL arena shooter prototype with three fixed-loadout classes, class skills, score/combo rules, enemy spawning, and an online leaderboard.',
    role: 'Game Programmer / Designer',
    period: '2026',
    platform: 'WebGL',
    href: 'https://kitchengun.github.io/wiki/games/last-mag/',
    tags: ['Unity', 'WebGL', 'Game Design', 'Cloudflare Workers'],
    status: 'Playable Prototype',
  },
  {
    title: 'UnrealMCP',
    description: 'MCP, Claude Skill, Codex, and Discord bot experiment for controlling Unreal editor tasks with natural language.',
    role: 'Tool Developer',
    period: '2026',
    href: 'https://github.com/KitchenGun/UnrealMCP',
    tags: ['Unreal Engine', 'MCP', 'AI Tools', 'C++'],
    status: 'Ongoing',
  },
  {
    title: 'hermes-hybrid',
    description: 'Hybrid LLM orchestrator with tier routing, policy gate, validator retry budget, Discord, and MCP gateway ideas.',
    role: 'Personal R&D',
    period: '2026',
    href: 'https://github.com/KitchenGun/hermes-hybrid',
    tags: ['Python', 'LLM', 'Agent', 'MCP'],
    status: 'R&D',
  },
  {
    title: 'personal-hermes-agent',
    description: 'Public sanitized operations profile and Job Registry for a personal AI agent, including workout/InBody logging, daily dev brief automation, and data-driven routine draft automation.',
    role: 'Personal R&D / Documentation',
    period: '2026',
    href: 'https://github.com/KitchenGun/personal-hermes-agent',
    tags: ['LLM', 'Agent', 'Automation', 'Fitness', 'Research'],
    status: 'Published',
  },
  {
    title: 'DX2D-METALSLUG',
    description: 'DirectX 2D Metal Slug clone project focused on C++ game programming fundamentals.',
    role: 'Game Programmer',
    href: 'https://github.com/KitchenGun/DX2D-METALSLUG',
    tags: ['C++', 'DirectX', '2D'],
    status: 'Archive',
  },
];
